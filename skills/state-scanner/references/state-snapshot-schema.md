...
- `true`  — Fetch 2 fetched `refs/aria/coordination` **and the local ref now equals the remote** (created or fast-forwarded; aria-plugin#197).
- `false` — benign absent: the ref is not published (NORMAL; no soft_error, no degraded).
- `null`  — unknown: Fetch 1 failed → Fetch 2 short-circuited, OR Fetch 2 failed non-benign — **including a local ref that is ahead of / diverged from the remote** (git rejects the non-forced update; soft_error `coordination_ref_fetch_failed` with label `non_ff`; the local ref is left untouched).

**Semantic change** (non-shape; aria-plugin#197, NOT a `snapshot_schema_version` bump): Fetch 2's refspec gained a destination (`refs/aria/coordination:refs/aria/coordination`). Before, git wrote only `FETCH_HEAD`, the local ref never moved, and `true` meant merely "the fetch command succeeded" — the documented freshness predicate (`success AND coordination_ref_present`, `layer-l-integration.md`) could pass on a days-old local view (observed 2026-09-12: 7 commits / 3 days behind origin). Now `true` implies the local view is level with the remote; a local-ahead or diverged ref yields `null` (fail-closed, unverified) instead of reporting fresh.
...
tracks_multibranch:
  ...
  collision:                      # TASK-000 (#133) — additive, ADVISORY-ONLY
    kind: str                     # enum: "none" | "cross_owner" | "self_multi_container"
                                   #   (session-handoff.md §2.3.5 by identity_key: cross_owner = >=2 identity_keys
                                   #    AND >=2 non-empty non-"unknown" owners; self_multi_container = >=2
                                   #    identity_keys AND <=1 attributable owner) — owner-container-identity-key
    groups: list[list[str]]       # per colliding track_id (D-0(a) family key: trailing "-<8hex>" stripped):
                                   #   list of ORIGINAL owner-container strings of the active members
    identity_advisories: list[dict]  # owner-container-identity-key — additive, ALWAYS present ([] when none):
                                   #   one per uuid identity_key seen under >=2 attributable owners across the
                                   #   RAW (pre-dedupe, cross-track) rows: {"identity_key": str,
                                   #   "owners": sorted list[str], "first_seen": str, "last_seen": str}
                                   #   ⚪ informational (§2.3.5 same-identity-multi-owner); never feeds kind/groups
...
Fix: before calling `classify()`, the collector calls the PUBLIC function `dedupe_latest_per_track_container(tracks) -> (deduped, stats)` (`handoff_multibranch.py`), which groups `tracks` by **`(track_id, identity_key)`** — `identity_key` per session-handoff.md §2.3.1 (`lib.collision.identity_key`: an 8-hex uuid container is the key by itself, so the same machine under two git identities folds — Aria #193; a hostname container keeps the owner segment, `owner/container`). Rows are parsed two-part `<owner>/<container-id>` via `lib.collision.split_owner_container` (owner-container-identity-key-and-collision-parser). The legacy SESSION segment (3rd part of an `owner/container/session` string) deliberately does **not** participate in the grouping key (round 3, finding [m]): two historical rows for the same track and the same physical container that differ only by session — e.g. an old `status: active` row from session `s1`, a newer `status: done` row from session `s2` — are the same container's history over time, not two containers, and must fold into one representative. Grouping on the raw `owner_container` string (rounds 1–2) left such session-differing rows in separate dedupe-of-one groups, so a stale `active` row could keep counting as a live candidate even after its own container had moved on. (Fail-soft: if the `lib.collision` import is unavailable, the grouping key falls back to the raw `owner_container` string — rounds 1–2 behaviour — rather than crashing; the session-drop fold does not apply in that degraded case.)

Within each `(track_id, identity_key)` group, the row that sorts greatest under the **four-level** compound key `(parse_ok, parsed updated_at, filename, branch)` wins:
...
`status == "legacy"` rows (no frontmatter; `owner_container` is always `"unknown"`) pass through un-grouped — `classify()` already excludes `owner_container == "unknown"` from attribution, so this has no observable effect on the classification result either way. Only the deduped view — further reduced by `lib.collision.filter_layer_h_fresh` (rows older than `LAYER_H_ACTIVE_WINDOW_DAYS` = 30 days dropped; owner-container-identity-key D-3(a), same single predicate the board uses) — is fed to `classify()`; `identity_advisories` is computed on the RAW rows BEFORE dedupe (dedupe folds exactly the same-machine multi-owner rows it must see); `tracks[]` is untouched.
...
| 2026-09-13 | aria-plugin#197 — Fetch 2 refspec gains a destination (`refs/aria/coordination:refs/aria/coordination`): `coordination_ref_present == true` now also means the local ref is level with the remote; local-ahead / diverged → `null` + soft_error `coordination_ref_fetch_failed` (label `non_ff`), local ref left untouched. Non-shape; schema stays `"1.0"`. |
