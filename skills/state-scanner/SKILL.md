...
**完整性兜底 (inter-cycle resume)**:

> snapshot 字段全 OK 不代表推荐充分。当所有以下条件**同时**成立时, AI 必须先 Read UPM 与 handoff doc 再下结论, 不得仅靠匹配的推荐规则触发输出:
> 1. `git.status_clean == true` (工作树干净, 无 in-flight 变更)
> 2. `audit.has_unconverged == false` (上次审计已收敛)
> 3. 无 `commit_only` / `quick_fix` 类高置信度规则匹配 (隐式排除 mid-feature 状态)
> 4. `requirements.upm.configured == true` 且 `requirements.upm.raw_block != null`
>
> 在该状态下, snapshot 现有字段可能漏掉 cross-session 的主线优先级信号 (UPM `## Pending Followups` 表、handoff doc `> 🚪 Next session 入口` 指针、in-progress US 的详细 raw_status), 仅匹配 `feature_new` 兜底规则会让用户感到"信息不完整"。
>
> AI 应主动:
> 1. **Read** `requirements.upm.source_file` 全文, 重点抓 `## Pending Followups` 表 (按 Priority 列排序的 P1/P2/P3 items)
> 2. **Grep** UPM 顶部 `> .*Next session 入口.*\((.+\.md)\)` 模式, 命中则 Read handoff doc
> 3. 用读到的优先级信息**覆盖**或**修正** snapshot 推荐, 而非简单输出 `feature_new`
>
> 此为 v3.0 collector 增强 (T1-T4) ship 前的过渡指引; T2/T3/T4 上线后 (Spec 待起) 此兜底降级为 sanity check。
