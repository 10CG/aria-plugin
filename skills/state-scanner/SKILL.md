...
<!-- 2026-04-23 新增 PRD 级元数据提取, 修复 #18 PRD Status extraction — 与 #17 (v1.16.1 regex heading-aware) 复用 Pattern 1-5 -->
...
Status 提取模式 (按优先级尝试):
  ...
  复用以上 Pattern 1-5 提取 PRD 自身的 status 字段 (大小写不敏感)。
  常见枚举值: Draft / Approved / Active / Deprecated / Archived

PRD 级元数据提取 (针对每个 prd-*.md 文件):
  path:           文件路径 (相对项目根)
  status:         复用 Pattern 1-5 提取 PRD 顶部的 Status 字段 (大小写不敏感)
                  未匹配时标记 "unknown"
  linked_stories: 扫描 user-stories/US-*.md 中 grep "prd-{basename}" 或
                  frontmatter `parent_prd: {basename}` 匹配到的文件数量
  launch_date:    从 PRD header 提取以下变体之一 (可选, 不存在留 null):
                    - /Launch Date:\s*(.+)/i
                    - /Target Launch:\s*(.+)/i
                    - /Soft Launch:\s*(.+)/i
...
输出 (已配置):
  requirements_status:
    configured: true
    prd_exists: true                       # 保留, backward compat
    prd_files:                             # 新增 (2026-04-23, fix #18)
      - path: "docs/requirements/prd-aria-v2.md"
        status: "Approved"
        linked_stories: 7
        launch_date: null
      - path: "docs/requirements/prd-phase3-commercial-launch.md"
        status: "Draft"
        linked_stories: 20
...
以及 PRD 状态: prd_draft_blocking (Draft PRD 关联 ≥5 Story 时优先推荐审阅拍板, fix #18)。
...
**最后更新**: 2026-04-23
**Skill版本**: 2.10.1 (2026-04-23: Phase 1.5 PRD 级元数据提取, fix #18)
