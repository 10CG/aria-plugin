...
Step 2 - 写 proposal.md (三路径分叉 + warn 覆盖层; 标记写入属本步, 非 Step 1 副作用):
  ...
  warn_overlay:
    ...
    与路径 (b) 共存: 若同批归档既 design-only 又 warn, frontmatter 同时含
      archive_type/archived_reason **和** unverified_claims/unverified_ack 四(+)个字段, 互不覆盖
    dry_run=true: 同路径 (c), 只在 dry-run 报告中 **回显** 若执行将写入的 unverified_claims 列表,
      不实际写入 (对齐 3c 逃生舱 dry-run 回显惯例)
