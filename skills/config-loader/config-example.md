...
```json
{
  ...
  "ai_native_estimator": {
    ...
    "min_samples": 3,
    "window": 10,
    "bootstrap_seed": { "L1": 30000, "L2": 150000, "L3": 500000 }
  },
...
### ai_native_estimator.*  (#18, v1.34.0+)

Token 轴工作量估算 (`ai-native-estimator` skill)。`enabled`（默认 `true`）控制 phase-d-closer 收尾是否自动 capture；`false` 时跳过。`min_samples`（默认 `3`）= forecast 返回 median 所需的同 spec_level 历史样本数，不足时返回 `insufficient` + uncalibrated bootstrap。`window`（默认 `10`）= `velocity()` 取最近多少 cycle。`bootstrap_seed`（按 spec_level）= 冷启动 N<min_samples 时的 work_metric 种子估算（tokens，显式标注 uncalibrated，仅参考）。
