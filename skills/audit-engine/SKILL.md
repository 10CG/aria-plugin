...
> **NOTE (#17 vs #79 边界)**: 本机制 (#17) 仅覆盖**审计讨论轮内 drift** (anchor 固化在单次审计周期内); 实施期偏离 Spec 的检测属 #79 (mid-implementation spec drift), 两者机制独立 (#79 文档待定, #17 单向 NOTE 暂可接受)。
...
关键字段:
  ...
  audit.drift_guard: object     # 多轮审计 Drift Guard (#17, v1.44.0)
                                ...
                                #   post_closure (max_rounds=1) 由模式选择阶段屏蔽, 不适用
