...
drift-checker **独立 30-60s 超时**, **不占 300s/轮 wall-clock** (并发控制条目见 audit-engine/SKILL.md §并发控制], 本文件不另造第二张并发表)。spawn 失败/超时 → fail-open: `drift_ratio=null` 按 `< warn` 档处理 + `drift_check_skipped: true`, 见 audit-engine/SKILL.md 错误处理表 (含与 `round_state.incomplete` 正交声明)。
