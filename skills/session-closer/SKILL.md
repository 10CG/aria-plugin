...
> **版本**: 1.0.1 | **定位**: 与十步循环**正交平级**的 leaf skill (会话维度, 非周期维度)
...
易混触发词的期望命中 (description 工程已据此 rebind: phase-d 删了「写 session handoff」+裸「收尾」):

| 触发词 | 命中 | 理由 |
|--------|------|------|
| "对话收尾" / "执行对话收尾" / "会话收尾" / "收工" / "session closeout" | **session-closer** | 纯会话语义 |
| "Phase D" / "归档 Spec" / "周期收尾" / "更新 cycle 进度" / "收尾阶段" | **phase-d-closer** | cycle 语义 |
| "写交接" / "写 handoff" | **session-closer (默认)** | cycle 内 handoff 由 phase-d D.3 **自动**承担 (用户不显式调); 用户显式说"写交接"多指会话维度 |

> 上表为**操作速查** (随插件自包含, 第三方无需 standards 子模块即生效)。完整方法论 (周期 vs 会话单元论证 + 5 层 handoff enforcement) 见 [standards/conventions/session-handoff.md §1.3](../../../standards/conventions/session-handoff.md) (方法论 SOT, 仅 vendor 了 aria-standards 的项目可达)。
...
**最后更新**: 2026-06-26 (v1.50.1: 触发消歧矩阵内联自包含 — 第三方无需 standards 子模块; 1.0.0: session-closer-synthesis DEC-20260625-001)
**Skill版本**: 1.0.1
