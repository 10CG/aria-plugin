...
- **`skills/audit-engine/scripts/sibling_spec_probe.py`** (新建, `scripts/` 目录新建, stdlib-only): 每轮审计入口扫远端仓**已落盘**的 proposal 语料 (`openspec/changes/*/proposal.md` + `openspec/archive/*/proposal.md`, enforced remotes × 全部 `refs/heads/*`), 与 claim 通道无共享失效模式 —— 对方没走认领或已 ship 归档时唯一还能把「这件事别人做完了」摆上台面的证据通道。四态 (姊妹纯函数 `state-scanner/lib/linked_issue_field.py`, 一条 E0–E6 都不复制) → 层分派 `canonical` / `none_sentinel` / `url_fallback` / `no_token_no_url` / `no_field` / `bad_token_union`; 键 `("k", <repo>, <n>)` (经 `normalize_linked_issue`) 或 `("r", <原串>)`, 集合求交; SC-19 常量黑名单 (SOT placeholder + 哨兵) 与姊妹同源。remote 集合取 `git remote` 配置面经 `resolve_enforced_remotes` (skill 级 → 顶层继承); 默认分支**只认** `git ls-remote --symref` (fail-closed, 不猜 master/main); `fetch --no-tags --prune` 进私有命名空间 `refs/aria/sibling-probe/<remote>/<branch>` (不动 `refs/remotes/*`, 不读 FETCH_HEAD, 不复用 remote_refresh 缓存), 每 git 子进程 30s, fetch 腿 ≤2 次。P11 分支维: `for-each-ref` 枚举 (默认 ref 先 + 字节序), 非默认 ref 数 `MAX_REFS_SCANNED=100`; 默认 ref 已归档同名 (`archive/<YYYY-MM-DD>-<spec_dir>/`, 日期前缀精确) ⇒ 分支副本视为陈旧跳过; (remote, corpus, spec_dir, key) 跨 ref 去重, `hits[].refs` 字节序。**Amendment A1**: `MAX_PROPOSALS_SCANNED=1000` 按**唯一 blob** 计 (ls-tree 默认格式取 sha, 同 blob 分类一次) —— 本仓 origin 9 分支 (ref, path) 1097 行 vs 165 blob, 逐行计数会让 cap 在宿主仓恒触发 ⇒ 每轮恒「未能核实」。stdout 恰一个 JSON (十二字段 `schema_version` / `probe` / `status` / `reason` / `verdict` / `own_spec_dir` / `own_layer` / `own_keys` / `remotes` / `hits` / `caps_applied` / `elapsed_ms`); `verdict` 一等字段 (`sibling_found` 当 hits 非空, 即使 degraded / `no_sibling_found` 仅覆盖完整且 own_keys 非空 / 其余 `not_established`), 消费方不得从 `hits == []` 推断; exit 0 = 有定义的判定 (含 degraded / skipped), 非 0 仅探针自身失败; no-silent-caps 三件套; `error_kind` 封闭集合 (照 `fetch_gate._classify_error` + `timeout` / `no_symref` / `bad_symref_prefix`), git 原始 stderr 永不回显 (Rule #7)。真仓实跑: 双远端 ok, origin 唯一 blob 155 / 9 ref / 陈旧 15, 41s。
- **`skills/audit-engine/tests/test_sibling_spec_probe.py`** (新建, `tests/` 目录新建, 104 条 unittest, `run_all_tests.sh` 自动纳入): SC-1~15 / 17~25 (SC-16 行为类无代码宿主, 由 AB 覆盖); 注入式 git runner (按 args 分派) / subprocess CLI 三终局十二键 / Rule #7 哨兵 / 主仓 `cc1bdef` 147 篇四臂 (行首 133/13/1 簇 3; 宽松 14 含假阳性; 仅头部 10/1 簇; 合成围栏夹具) / SC-21 干净子解释器 import 顺序; 四个坏实现负控亲跑均红 (只扫 changes / 只枚举默认 ref / 层 2 触发改「集合为空」/ import 块反序)。
- **指令面** (Rule #6 处方性): `references/execution-modes.md` Convergence 与 Challenge 两围栏块各插逐字两行「每轮入口」调用串 (SC-17 计数恰 2; 下游 Level-3 走 Challenge 不漏) + 新节 `## 竞品 spec 探针 (per-round 入口)` = 契约权威版 (十二字段 / verdict 表 / exit / 三档措辞 / 消费方 fail-closed 义务); `SKILL.md` 新小节「per-round 入口探针」(与 Step 0「Round 1 前一次性」显式消歧, 不沿用 Step 编号; α 每轮调用并渲染进 `### Round N` / β `not_established` · exit≠0 · 非 JSON ⇒ 「未能核实」而非「无竞品」; 不阻断 / 不改 verdict / 不改收敛); `references/report-format.md` Round 1/2/N 模板 `- Sibling probe:` 行 (🔴 检测到 N 份同 issue 的竞品 Spec (含 M 份已完成的 Spec) | 已完整扫描, 未发现同 issue 竞品 | 未能核实 (原因: <reason>))。
- **Rule #6 照跑**: 主仓新建 `aria-plugin-benchmarks/ab-suite/audit-engine.json` (α/β 两 eval, descriptive, 经 /skill-creator eval 流程; version.yaml 1.3.0, 32 skill / 76 case 程序化重算); AB 实跑 with_skill **8/8** vs old_skill (v1.68.2 快照) **3/8**, delta **+0.62**, WITHOUT_BETTER 0, 四臂 descriptive 形态一致 —— `aria-plugin-benchmarks/ab-results/2026-09-03-v1.69.0-sibling-spec-probe-rule6/`。
- **不改** `skills/state-scanner/**` (只 import 其已 ship 契约); `audit-engine/` 下不新建 `lib/` / `collectors/`。已知限: 私有 ref 命名空间无 GC (每 remote/branch 一条, 量级小); `MAX_PROPOSALS_SCANNED` 对第三方仓未验证 (刻意不做 config key); 本 Spec 自己的字段是哨兵 `none` ⇒ 对本探针永不命中 (设计如此)。

**含 [1.68.2]** (下方 PATCH, 同批 ship)。审计: post_spec R1–R6 / post_planning R1–R4 CONVERGED (主仓 `.aria/audit-reports/post_planning-R4-*-a1-entry-combined-*`); B 期技术裁定 D1–D6 见主仓决策单 `2026-09-01-a1-entry-h1-h6-technical-rulings-product-vs-technical-split.md` §2026-09-03。
...
- **并发说明**: v1.65.2/v1.65.3 由并发 track 占用 (#124/#125/#126 path_coverage 修复), 本轨版本顺延至 1.65.4; 两轨改动面零重叠 (hooks vs phase-c-integrator)。
